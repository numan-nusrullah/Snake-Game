...
class App extends Component {
  constructor() {
    ...
    componentDidMount = () => {
      ...
      setTimeout(() => {
        this.gameLoop()
      },this.state.snake.tail.length ? (1000 / this.state.snake.tail.length) + 200 : 500);
    }
    getRandomApple = () => {
    ...
    gameLoop = () => {
      ...
      this.setState(({ snake , apple})=> {
        ...
        setTimeout(() => {
          this.gameLoop()
        },this.state.snake.tail.length ? (500 / this.state.snake.tail.length) + 200 : 500);
      });  
    }
  ...
  render() {
    ...
    return (
      <div className="App">
       {
         gameOver ? <div> <h1 id='score'> Game Over ! You Scored {snake.tail.length }</h1> 
           <Restart />
         </div>
        : <section className="grid ">
        {
          grid.map((row, i)=>(
            row.map(cell => (
              <div key={`${cell.row} ${cell.col}`} className={`cell ${this.isHead(cell) ? 'head' : this.isApple(cell) ? 'apple' :  this.isTail(cell) ? 'tail' : ''}`}>
              </div>
            ))
          ))
        }
